/**
 * `multi` question type — one or more correct options.
...
module.exports = {
  ...
  validate(q) {
    ...
    if (correct.length < 1) return "multi questions need at least one correct option";
    if (new Set(correct).size !== correct.length) return "correct must not repeat option ids";
